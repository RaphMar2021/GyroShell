...
namespace GyroShell.Controls
{
    public sealed partial class DefaultTaskbar : Page
    {
        ...
        #region Taskbar Icons Position
        private void TaskbarCenter()
        {
            DispatcherTimer tbPosDT = new DispatcherTimer();
            tbPosDT.Tick += TbPosDT_Tick; ;
            tbPosDT.Interval = new TimeSpan(0, 0, 1);
            tbPosDT.Start();
        }

        private void TbPosDT_Tick(object sender, object e)
        {
            if (((bool)App.localSettings.Values["tbCenter"]))
            {
                LeftControls.HorizontalAlignment = HorizontalAlignment.Center;
                LeftControls.Margin = new(0, 0, 0, 0);
            }
            else
            {
                LeftControls.HorizontalAlignment = HorizontalAlignment.Left;
                LeftControls.Margin = new(9, 0, 0, 0);
            }
